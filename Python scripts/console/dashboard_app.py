...
class DashboardApp:
    ...
    def initialize_device(self):
        try:
            ul.ignore_instacal()  # Ignore InstaCal configurations
            devices = ul.get_daq_device_inventory(InterfaceType.USB)
            if len(devices) > 0:
                device = devices[0]  # Assuming the first device should be used
                try:
                    ...
                    # Attempt to release and reconnect if the first attempt fails
                    messagebox.showerror("Device Connection Error", f"Failed to connect on first attempt: {e}. Trying again...")
                    ul.release_daq_device(self.board_num)
                    ul.create_daq_device(self.board_num, device)
                    self.device_available = True
                    messagebox.showinfo("Device Info", "Successfully connected on second attempt.")
            else:
                self.device_available = False
                messagebox.showwarning("Device Warning", "No DAQ devices found. Ensure the device is connected and recognized by the system.")
        except ULError as e:
            self.device_available = False
            messagebox.showerror("Initialization Error", f"An error occurred during device initialization: {e}")
